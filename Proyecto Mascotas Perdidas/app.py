...
def login():
    ...
    if request.method == "POST":
        ...
        if username == ADMIN_USER and password == ADMIN_PASS:
            session["admin"] = True
            session["usuario"] = (
                username  # Guardar usuario en sesión para mostrar nombre
            )
        ...
        if usuario and usuario.check_password(password):
            ...
            session["usuario"] = (
                usuario.username
            )  # Guardar el username para mostrar en header
...
def registro():
    mensaje = None
    tipo_mensaje = "none"  # Por defecto, no hay mensaje
    if request.method == "POST":
        username = request.form.get("username")
        password = request.form.get("password")
